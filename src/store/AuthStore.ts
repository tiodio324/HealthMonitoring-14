...
const AUTH_STORAGE_KEY = 'health_monitoring_auth';
const SESSION_EXPIRY_KEY = 'health_monitoring_session_expiry';
const SESSION_DURATION = 24 * 60 * 60 * 1000;
const AUTH_CREDENTIALS: Record<Exclude<UserRole, 'viewer'>, string> = { doctor: 'doctor2026-health', admin: 'admin2026-health' };
...
export class AuthStore {
  ...
  get isDoctor(): boolean { return this._user.role === 'doctor' || this._user.role === 'admin'; }
  ...
  canManagePatients = (): boolean => this.permissions.canManagePatients;
  canManageMeasurements = (): boolean => this.permissions.canManageMeasurements;
  canAccessAdmin = (): boolean => this.permissions.canAccessAdmin;

  hasRole = (r: UserRole): boolean => ({ viewer: 0, doctor: 1, admin: 2 }[this._user.role] >= { viewer: 0, doctor: 1, admin: 2 }[r]);
